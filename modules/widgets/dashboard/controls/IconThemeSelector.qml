...
Item {
    ...
    readonly property string selectedTheme: Config.theme.iconTheme || systemTheme
    ...
    function selectTheme(themeId) {
        if (!themeId)
            return;
        if (Config.theme.iconTheme === themeId)
            return;
        GlobalStates.markThemeChanged();
        Config.theme.iconTheme = themeId;
    }
    ...
    Process {
        ...
        command: [
            "python3",
            decodeURIComponent(Qt.resolvedUrl("../../../../scripts/list_icon_themes.py").toString().replace("file://", ""))
        ]
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    console.error("IconThemeSelector: failed to parse themes:", e);
                    root.themes = [];
                }
            }
        }
        stderr: StdioCollector {
            onStreamFinished: err => {
                if (err)
                    console.error("IconThemeSelector scan error:", err);
            }
        }
    }
