...
Singleton {
    ...
    function runInActiveWorkspace(command) {
        ...
        processComponent.command = ["bash", "-c", "cd ~ && env -u HL_INITIAL_WORKSPACE_TOKEN -u ELECTRON_RUN_AS_NODE -u ELECTRON_NO_ATTACH_CONSOLE setsid " + command + " < /dev/null > /dev/null 2>&1 &"];
